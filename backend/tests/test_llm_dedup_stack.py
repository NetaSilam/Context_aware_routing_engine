...
def test_a_post_with_no_coordinates_never_gets_a_dedup_check_job() -> None:
    with _signup_client() as client:
        created = _create_post(client, hazard_type="crash", longitude=None, latitude=None)
        status, _ = _wait_for_job(created["id"], "triage")
        assert status == "completed"

        job = _job_row(created["id"], "dedup_check")
