...
run_service() {
  ...
  "${compose[@]}" up --build --wait --scale worker=2 postgres redis fake-osrm fake-geocoder api worker llm-worker-fast llm-worker-slow llm-dedup-api llm-dedup-worker-fast frontend abuse-api stress-api secure-auth-api auth-unavailable-api geocoding-unavailable-api queue-unavailable-api
