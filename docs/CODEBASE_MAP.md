...
| Change the live forum feed / DM thread (as opposed to personal notifications above) | `backend/app/notifications/service.py`'s `publish_forum_activity`/`FORUM_ACTIVITY_CHANNEL`, `forum/routes.py`'s `GET /api/forum/activity/stream` and its `publish_forum_activity` call sites, `frontend/src/pages/ForumPage.tsx` and `MessagesPage.tsx`'s `EventSource` effects — see `FORUM_FEATURE_PRD.md` decision 27 |
...
| Add a component that opens an `EventSource` | jsdom has no native `EventSource` — `frontend/src/test-setup.ts` (wired via `vite.config.ts`'s `test.setupFiles`) provides a global no-op default so rendering doesn't throw; add a local mock (see `NotificationIndicator.test.tsx`/`ForumPage.test.tsx`) in the component's own test file to assert actual live-update behavior |
